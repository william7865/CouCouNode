        ...
        <div key={category.id} className="mb-6 px-4">
          ...
          <div className="flex space-x-4 overflow-x-auto p-2">
            {category.films.map((film, index) => (
              <Link key={index} href={`/movie/${film.title.toLowerCase().replace(/\s+/g, "-")}`}>
                <img src={film.image} alt={film.title} className="w-40 h-60 object-cover rounded-lg cursor-pointer" />
              </Link>
            ))}
          </div>
        </div>
