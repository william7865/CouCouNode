...
const WatchButton = ({ platform, url }) => {
  if (!url) return null;
  ...
  return (
    <a
      ...
      className={`${config.color} text-white px-4 py-2 rounded flex items-center gap-2 font-bold transition-colors`}
    >
      {config.icon}
  ...
  const fetchSeries = useCallback(async () => {
    try {
      ...
      const formattedData = data.map(serie => ({
        ...
        streaming_links: {
          netflix: serie.netflix_url || null,
          youtube: serie.youtube_url || null
        },
        episodes: Array.isArray(serie.episodes) ? serie.episodes.map(ep => ({
          ...ep,
          streaming_links: {
            netflix: ep.netflix_url || null,
            youtube: ep.youtube_url || null
          },
            ...
            <div className="bg-gray-900 rounded-lg overflow-hidden shadow-2xl">
              ...
              <div className="p-6">
                <div className="flex flex-wrap items-center gap-4 mb-6">
                  {selectedSerie.streaming_links.netflix && (
                    <WatchButton 
                      platform="Netflix" 
                      url={selectedSerie.streaming_links.netflix} 
                    />
                  )}
                  {selectedSerie.streaming_links.youtube && (
                    <WatchButton 
                      platform="YouTube" 
                      url={selectedSerie.streaming_links.youtube} 
                    />
                  )}
                ...
                {activeTab === "episodes" && (
                  <div className="space-y-6">
                    <h2 className="text-2xl font-bold">
                      Saison {selectedSerie.episodes?.[0]?.season_number || 1}
                    </h2>
                      ...
                      >
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex gap-3 mt-4">
                            {episode.streaming_links.netflix && (
                              <WatchButton 
                                platform="Netflix" 
                                url={episode.streaming_links.netflix} 
                                className="text-sm"
                              />
                            )}
                            {episode.streaming_links.youtube && (
                              <WatchButton 
                                platform="YouTube" 
                                url={episode.streaming_links.youtube} 
                                className="text-sm"
                              />
                            )}
                          </div>
        ...
        <div className="relative z-20 h-full flex flex-col justify-center pb-32 px-8">
          <div className="max-w-2xl">
            ...
            <div className="flex gap-4">
              {featuredSerie.streaming_links.netflix && (
                <WatchButton 
                  platform="Netflix" 
                  url={featuredSerie.streaming_links.netflix} 
                />
              )}
              {featuredSerie.streaming_links.youtube && (
                <WatchButton 
                  platform="YouTube" 
                  url={featuredSerie.streaming_links.youtube} 
                />
              )}
              <button 
